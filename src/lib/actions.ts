...
export async function createClient(formData: FormData) {
  try {
    ...
    // Ideally we should handle this in the UI with useFormState
    // For now, let's keep the redirect but maybe we could append an error param
  }
  redirect('/clients');
}

export async function createClientQuick(data: { name: string, phone?: string, address?: string }) {
  try {
    ...
    console.error('Error in createClientQuick:', error);
