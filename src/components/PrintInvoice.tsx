      ...
      <div className="max-w-[190mm] mx-auto !text-black">
        ...
        <div className="flex justify-between items-start border-b-4 border-black pb-8 mb-10">
          <div className="space-y-2">
            ...
            <p className="text-sm font-bold !text-black uppercase tracking-widest">Premium Beverage Delivery System</p>
          ...
          <div className="text-right">
            ...
            <div className="space-y-1">
              <p className="text-[10px] font-black !text-black uppercase tracking-widest">Documento Nº</p>
              <p className="text-2xl font-black tabular-nums !text-black">#INV-{sale.id.toString().padStart(5, '0')}</p>
              <p className="text-xs font-bold !text-black pt-2">{new Date(sale.createdAt).toLocaleString('pt-BR')}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-12 mb-12">
          <div className="border-l-4 border-black pl-6 space-y-4">
            <h3 className="text-xs font-black uppercase tracking-widest !text-black">Dados do Cliente</h3>
            {sale.client ? (
              <div className="space-y-2">
                <div>
                  <p className="text-[9px] font-black !text-black uppercase">Nome / Razão Social</p>
                  <p className="text-sm font-bold uppercase !text-black">{sale.client.name}</p>
                </div>
                {sale.client.phone && (
                  <div>
                    <p className="text-[9px] font-black !text-black uppercase">Contato</p>
                    <p className="text-sm font-bold !text-black">{sale.client.phone}</p>
                  </div>
                )}
                {sale.client.address && (
                  <div>
                    <p className="text-[9px] font-black !text-black uppercase">Endereço de Entrega</p>
                    <p className="text-sm font-bold !text-black">{sale.client.address}</p>
                  </div>
                )}
              </div>
            ) : (
              <p className="text-sm font-bold italic !text-black">Consumidor Final (Venda de Balcão)</p>
            )}
          </div>
          <div className="border-l-4 border-black pl-6 space-y-4">
            <h3 className="text-xs font-black uppercase tracking-widest !text-black">Detalhes do Pagamento</h3>
            <div className="space-y-2">
              <div>
                <p className="text-[9px] font-black !text-black uppercase">Método Utilizado</p>
                <p className="text-sm font-bold underline underline-offset-4 decoration-2 !text-black">{sale.paymentMethod}</p>
              </div>
              <div>
                <p className="text-[9px] font-black !text-black uppercase">Status do Documento</p>
                ...
                <tr key={idx}>
                  <td className="py-4 px-3">
                    ...
                    <div className="flex gap-2 items-center mt-1">
                      {item.product.brand && <span className="text-[9px] font-bold !text-black uppercase">{item.product.brand}</span>}
                      {item.product.volume && <span className="text-[9px] font-bold !text-black uppercase">{item.product.volume}</span>}
                    </div>
                    {item.product.barcode && (
                      <p className="text-[8px] font-mono !text-black mt-1">REF: {item.product.barcode}</p>
                    )}
                  </td>
        ...
        <div className="flex justify-end mb-20">
          <div className="w-80 space-y-3">
            <div className="flex justify-between items-center text-xs font-bold !text-black border-b border-dashed border-black pb-2">
            ...
            {sale.discount > 0 && (
              <div className="flex justify-between items-center text-xs font-black border-b border-dashed border-black pb-2 italic">
        ...
        <div className="border-t-2 border-black/10 pt-10 text-center space-y-2">
          <p className="text-[10px] font-black uppercase tracking-[0.3em] !text-black">Obrigado pela sua compra!</p>
          <p className="text-[8px] font-bold !text-black uppercase tracking-widest">Aurora Engine - Sistema de Gestão Premium de Distribuidoras</p>
        </div>
      </div>
    </div>
  );
